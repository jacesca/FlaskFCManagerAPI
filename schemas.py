...
class TeamSchema(PlainTeamSchema):
    id = fields.Int(
        dump_only=True
    )  # dump_only means it can not be modifie or created.  # noqa
    league = fields.Str(required=True)
    club_id = fields.Int(
        required=True, load_only=True
    )  # load_only means that this field can not be output by the get method but it only can be reached or modified by the post method  # noqa
    players = fields.List(fields.Nested(PlainPlayerSchema()), dump_only=True)
    managers = fields.List(
        fields.Nested(PlainUserSchema()), dump_only=True
    )  # many to may relationship  # noqa
